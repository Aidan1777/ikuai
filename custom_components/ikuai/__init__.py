...
class IKUAIDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def async_control_device(self, action_body):
        ...
        if self._allow_login:
            ...
            if not sess_key:
                return
            header = {'Cookie': f'username={self._fetcher._username}; login=1; sess_key={sess_key}', 'Content-Type': 'application/json;charset=UTF-8'}
            result = await self._fetcher.requestpost_json(self._fetcher._host + "/Action/call", header, action_body)
            if result == 401:
                self._token_expire_time = 0
            return result
